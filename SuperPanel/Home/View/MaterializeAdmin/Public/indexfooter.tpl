<style>

</style>
<footer class="page-footer-index">
    <div class="container">
        <div class="row">
            <div class="col l6 s12">
                <h5 class="white-text">通知:</h5>
                <p class="grey-text text-lighten-4"><{$announcement['footer']}></p>
            </div>
            <div class="col l4 offset-l2 s12">
                <h5 class="white-text">链接</h5>
                <ul>
                  <li><a class="grey-text text-lighten-3" href="#!">链接 1</a></li>
                  <li><a class="grey-text text-lighten-3" href="#!">链接 2</a></li>
                  <li><a class="grey-text text-lighten-3" href="#!">链接 3</a></li>
                  <li><a class="grey-text text-lighten-3" href="#!">链接 4</a></li>
                </ul>
              </div>
            </div>
          </div>
          <div class="footer-copyright">
            <div class="container">
            © 2016 Copyright <{$site_name}>
            <a class="grey-text text-lighten-4 right" href="#!">Powered By SuperPanel 1.0 Demo</a>
            </div>
          </div>
        </footer>