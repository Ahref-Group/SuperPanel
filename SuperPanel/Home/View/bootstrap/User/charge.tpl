                                                ...
                                                <foreach name="alipay_order_list" item="order">
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            <{$order['status']=='TRADE_FINISHED' ? "<i class='fa fa-check'></i>": "<i class='fa fa-close'></i>"}>
                                                        </td>
                                                        <td>
                                                            <{$order.creat_time}>
                                                        </td>
                                ...
                                <div class="panel-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-7 col-sm-8">
                                            <div class="form-group">
                                                <input class="form-control" type="text" id="exchange-code">
                                            </div>
                                        </div>
                                        <div class="col-md-2 col-sm-12">
                                            <button class="btn btn-primary btn-block" id="check-exchange-code">检查</button>
                                        </div>
                                    </div>
                                    <hr />
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <div class="well text-center text-primary" id="exchange-code-info">请先检查兑换码</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="panel-body">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered table-hover">
                                            ...
                                            <tbody>
                                                <foreach name="exchange_list" item="exchange">
                                                <tr>
                                                    <td><{$key+1}></td>
                                                    <td><{$exchange.exchange_time}></td>
                                                    <td><{$exchange.money}> 喵币</td>
                                                </tr>
                                                </foreach>
                                            </tbody>
                                        </table>
...
<script>
    ...
    $("#check-promo-code").click(function() {
        if (($("#promo-code").val()).length != 0) {
            ...
            $.post("<{:U("Home/UserAction/checkPromoCode")}>", {
    ...
    $('#exchange').click(function(){
        exchange_code = $('#exchange-code').val();
        $.post("<{:U('Home/UserAction/exchange')}>", {exchange_code:exchange_code}, function(data){
            if(data['status'] == 'success'){
                $.alert("兑换成功，当前您的余额为："+data['info']);
            }else{
                $.alert("兑换失败，原因是："+data['info']);
            }
        });
    });
    
    <if condition = "$charge_status" >
        $.confirm({
            title: '充值成功',
